...
This package goes with PROGEDO-Loire formations (https://msh-progedo-loire.univ-nantes.fr/). It aims to make easier exploratory analysis of quantitative data in social sciences.

## Installation

You can install the development version of exploquantir from GitHub with:
...
Returns a *gt* table displaying contingency analysis results (from two categorical variables of a dataframe)
...
Calculates and plots proportions of numeric variables variances explained by categorical variables
...
Returns a dataframe from which we can easily plot a MCA graph using *ggplot*. This function is inspired by [Anton Perdoncin, "*Représenter graphiquement les résultats d’une analyse factorielle avec R*"](https://quanti.hypotheses.org/930).

This dataframe must be named "***modalites***" to work with *code.ex* argument. Indeed, if *code.ex* is TRUE, the function displays an example of ggplot code in the Console.
...
Returns a *dataframe* from which we can easily get indexes of "junk" categories, so as to achieve a specific MCA with FactoMineR.
